...
float ColorHistogram::compareHist(ColorHistogram h2,int method=comparison_method::NORM_L1)
{
  ...
  for( size_t i = 0; i < it.nplanes; i++, ++it )
  {
    ...
    else if( method == comparison_method::BHATTACHRYA || method == comparison_method::BHATTACHRYA1 )
    {
      ...
      std::cerr << (float)s1 <<":" << (float)s2 << std::endl;
...
Mat ColorHistogram::getHist()
{
  return _histMat;
}

Mat ColorHistogram::likeyhoodImage(Mat image)
{
...
void ColorHistogram::setMask(Mat mask)
{
  mask.copyTo(_mask);
}


void ColorHistogram::mergeHistogram(Mat hist,float factor)
{
  if(_histMat.empty())
  {
...
cv::Mat ColorHistogram::BuildHistogram(cv::Mat srcImage,bool accumulate){
  cv::Mat histMat;
  //compute  histogram
  int *c=(int *)calloc(sizeof(int),_channels.size());
  for(int i=0;i<_channels.size();i++)
...
Mat ColorHistogram::drawHist(MatND _histMat)
{
...
void ColorHistogram::setChannel(vector<int>channels)
{
  _channels=channels;

}

void ColorHistogram::setHistSize(vector<int> size)
{
  _histSize=size;
}

void ColorHistogram::setRange(vector<float> range)
{
  _histRange=range;
}

Mat ColorHistogram::applyKernel(int size,int type)
{
...
std::vector<int> ColorHistogram::getThreshHist(cv::Mat histMat, float s1, float s2){
...
std::vector<int> ColorHistogram::getThresh(cv::Mat image, float s1, float s2){
...
/*
   Mat ColorHistogram::findmode(Mat a=Mat(),int current)
   {
   Mat hist;
   if(a.rows>0)
   a.copyTo(hist);
